...
# Models are baked into the image at build time; fall back to HF hub if missing.
...
class ModelManager:
    ...
    def load(self, model_id: str) -> _CachedModel:
        ...
        if local.exists():
            ...
            # Lazy import: optimum + torch only needed if falling back to HF export
    ...
    def _run(session: ort.InferenceSession, inputs: dict) -> np.ndarray:
        """Run a single forward pass; returns logits array of shape [num_labels]."""
    ...
    def _predict_truncate(text: str, entry: _CachedModel) -> tuple[str, float, int]:
        ...
        logits = ModelManager._run(entry.session, inputs)
    ...
    def _predict_sliding(
        ...
        if total_len <= WINDOW_SIZE:
            logits = ModelManager._run(entry.session, encoding)
            scores = ModelManager._softmax(logits)
            label = ID2LABEL[int(scores.argmax())]
            return label, float(scores.max()), 1
        ...
        while start < total_len:
            ...
            logits = ModelManager._run(entry.session, chunk_inputs)
            scores = ModelManager._softmax(logits)
